...
class TestAutobus {
	...
	//********* Exception nombre place ***************
	System.out.print('.'); nbTest++;
	new TestAutobus().testNbPlaceNegative();
	...
	for (int i = 1; i < 4; i++) { //3
	    bus.monteeDemanderAssis(faux[i]);
	    faux[i].status = Position.ASSIS;
	}

	for (int i = 4; i <= 7; i++) { //4
	    bus.monteeDemanderDebout(faux[i]);
	    faux[i].status = Position.DEBOUT;
	}
	...
	faux[1].status = Position.ASSIS;

	bus.monteeDemanderDebout(faux[2]);
	faux[2].status = Position.DEBOUT;

	bus.monteeDemanderDebout(faux[3]);
	faux[3].status = Position.DEBOUT;

	bus.monteeDemanderAssis(faux[4]);
	faux[4].status = Position.ASSIS;

	bus.monteeDemanderDebout(faux[5]);
	faux[5].status = Position.DEBOUT;

	bus.monteeDemanderAssis(faux[6]);
	faux[6].status = Position.ASSIS;

	bus.monteeDemanderAssis(faux[7]);
	faux[7].status = Position.ASSIS;
    ...
    protected void testNbPlaceNegative(){
	try{
	    new Autobus(-1,2);
	    assert false : "Exception nb place negative pas catch";
	}
    ...
    protected void testMonteeDouble(){
	Autobus a = new Autobus(2,2);
	FauxPassager p1 = new FauxPassager();
	a.monteeDemanderAssis(p1);
	try{
	    a.monteeDemanderAssis(p1);
	    assert false : "Exception double montee pas catch";
	}
